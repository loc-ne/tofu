...
# --- 1. HÀM LOAD MÔ HÌNH (CÓ CACHE VÀ ÉP LÊN GPU) ---
@st.cache_resource
...
# --- 🔥 CƠ CHẾ AUTO-LOAD: TỰ ĐỘNG NẠP MÔ HÌNH NGAY KHI LÊN WEB ---
# Khối này tự chạy ngầm khi khởi động giao diện mà không cần bấm nút
if model1_name and model2_name:
    path1 = os.path.join(models_dir, model1_name)
    path2 = os.path.join(models_dir, model2_name)
    
    # Kích hoạt nạp ngầm hai mô hình lên VRAM GPU
...
if st.button("🚀 Chạy Demo (Generate)", type="primary"):
    if question and model1_name and model2_name:
        ...
        # Cột 1: Fine-tuned Model (Đã được nạp sẵn)
        with col1:
        ...
        # Cột 2: Unlearned Model (Đã được nạp sẵn)
        with col2:
