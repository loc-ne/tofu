...
# Add parent directory to sys.path to import data_module
...
def main():
    ...
    for idx, item in enumerate(full_data):
        ...
        stop_words_ids = [tokenizer.encode("\n")[0], tokenizer.eos_token_id]
        with torch.no_grad():
            outputs = model.generate(
                ...
                # --- BẬT CHẾ ĐỘ SAMPLING ĐỂ CỨU CÂU TRỐNG ---
                do_sample=True,                # Chuyển sang True để mô hình linh hoạt chọn từ
                temperature=0.4,               # Để thấp (0.3 - 0.4) để câu chữ vẫn bám sát Ground Truth, không bị bịa chuyện Lily
                top_k=20,                      # Chỉ chọn từ top 20 từ có xác suất cao nhất
                top_p=0.85,                    # Giới hạn phân phối xác suất tích lũy
                
                # --- GIỮ CẤU HÌNH CHẶN LẶP VÀ NGẮT ---
