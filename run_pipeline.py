...
def main():
    ...
    methods = {
        "GA":  {"loss": "grad_ascent", "name": "grad_ascent", "lr": "2e-5"},
        "GD":  {"loss": "grad_diff",   "name": "grad_diff",   "lr": "2e-5"},
        "KL":  {"loss": "KL",          "name": "KL",          "lr": "2e-5"},
        "DPO": {"loss": "dpo",         "name": "dpo",         "lr": "2e-5"},
    }
    ...
    for key, info in methods.items():
        ...
        eval_path  = os.path.join(BASE_DIR, split, f"phi_unlearn_{key}_{RUN_TAG}")
        csv_path   = os.path.join(BASE_DIR, split, f"stat_{key}_{RUN_TAG}.csv")
    ...
    csv_files = " ".join([
        os.path.join(BASE_DIR, split, f"stat_{key}_{RUN_TAG}.csv")
    ...
    print(f"  Run tag : {RUN_TAG}")
    print(f"  CSVs    : {split}/stat_*_{RUN_TAG}.csv")
    print(f"  Charts  : {split}/unlearn_curves.png")
    print(f"            {split}/tradeoff_scatter.png")
    print("="*80 + "\n")
